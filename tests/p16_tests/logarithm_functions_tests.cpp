...
TEST(Posit16MathFunctions, LnFunction) {
  std::uniform_int_distribution<uint16_t> full_dist(
      0, 0x7FFF); // only positive values
  ...
  for (int i = 0; i < NTESTS16; i++) {
    ...
    ASSERT_TRUE(double_eq(p_result.toDouble(), p_expected.toDouble(), 1e-1))
        << "Failed ln: ln(" << f_a << ") = " << p_result.toDouble()
        << " but expected " << p_expected.toDouble();
  }
}
...
TEST(Posit16MathFunctions, Log2Function) {
  ...
  for (int i = 0; i < NTESTS16; i++) {
    ...
    ASSERT_TRUE(double_eq(p_result.toDouble(), p_expected.toDouble(), 1e-1))
        << "Failed log2: log2(" << f_a << ") = " << p_result.toDouble()
        << " but expected " << p_expected.toDouble();
  }
}