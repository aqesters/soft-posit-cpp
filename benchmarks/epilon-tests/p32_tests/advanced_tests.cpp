...
TEST(Posit32Advanced, AddSubCancel)
{
    ...
    std::uniform_int_distribution<int32_t> safe_dist(INT32_MIN / 2, INT32_MAX / 2);
...
TEST(Posit32Advanced, DistributiveProperty)
{
    ...
    std::uniform_int_distribution<int32_t> safe_dist(INT32_MIN / 4, INT32_MAX / 4);

    for (int i = 0; i < NTESTS32 / 20; i++)
    {  // Further reduced iteration count
        ...
        ASSERT_TRUE(double_eq(left_side.toDouble(), right_side.toDouble(), 1e-12, 1e-1))
