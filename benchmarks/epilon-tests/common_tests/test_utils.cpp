...
double find_smallest_epsilon(double a, double b, double epsilon_min, double epsilon_max)
{
...
void printEpsilonStats(const std::string &label, const std::vector<double> &epsilons)
{
    ...
    double sq_sum  = std::inner_product(epsilons.begin(),
                                       epsilons.end(),
                                       epsilons.begin(),
                                       0.0,
                                       std::plus<double>(),
...
void EpsilonStatisticsPrinter::OnTestProgramEnd(const ::testing::UnitTest &unit_test)
{
    ...
    if (!operation_epsilons.empty())
    {
        ...
        for (const auto &pair : operation_epsilons)
        {
            ...
            std::cout << op << "\t" << epsilons.size() << "\t" << min << "\t" << median << "\t"
                      << mean << "\t" << max << "\t" << std_dev << std::endl;
        }
    }
}